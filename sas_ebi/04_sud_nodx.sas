...
%macro SUD_NODX;
	...
	***** ADDITIONAL QC: LOOK AT HOW MANY CLAIMS IDENTIFIED IN ABOVE METHODS HAVE ALL DIAGNOSIS CODES = NULL *** ;

	%macro pull_inp(fltype,ndiags);

		execute (
			create temp table &fltype._method3 as
			select submtg_state_cd,
			       msis_ident_num,
				   &fltype._link_key,
				   max(INP_SUD) as INP_SUD

			from &fltype._inp_sud_nodx
			group by submtg_state_cd,
			         msis_ident_num,
					 &fltype._link_key
		...
		** Join back to headers to get diagnosis codes;

		execute (
			create temp table &fltype._method3b as
			...
			from (select * from &fltype._method3 where INP_SUD=1) a
			     left join
				 &fltype.H b
		...
		title2 "ADDITIONAL QC: % OF &fltype. inpatient records identified as INP_SUD with all null diagnosis codes";

		%crosstab(&fltype._method3b,dgns_null);

		select * from connection to tmsis_passthrough
		(select dgns_null %do i=1 %to &ndiags.; , dgns_&i._cd %end; from &fltype._method3b where dgns_null=1 limit 100);

		select * from connection to tmsis_passthrough
		(select dgns_null %do i=1 %to &ndiags.; , dgns_&i._cd %end; from &fltype._method3b where dgns_null=0 limit 100);
	...
	** Now do the same but for LT and OT outpatient SUD claims;

	%macro pull_outp(fltype,ndiags);


		execute (
			create temp table &fltype._method3o as
			select submtg_state_cd,
			       msis_ident_num,
				   &fltype._link_key,
		...
		** Join back to headers to get diagnosis codes;
		...
		execute (
			create temp table &fltype._method3bo as
			select a.submtg_state_cd,
                   a.&fltype._link_key
				   %do i=1 %to &ndiags.;
				   	  ,dgns_&i._cd
			...
			from (select * from &fltype._method3o where OUTP_SUD=1) a
			     left join
				 &fltype.H b
		...
		title2 "ADDITIONAL QC: % OF &fltype. outpatient records identified as OUTP_SUD with all null diagnosis codes";

		%crosstab(&fltype._method3bo,dgns_null);

		select * from connection to tmsis_passthrough
		(select dgns_null %do i=1 %to &ndiags.; , dgns_&i._cd %end; from &fltype._method3bo where dgns_null=1 limit 100);

		select * from connection to tmsis_passthrough
		(select dgns_null %do i=1 %to &ndiags.; , dgns_&i._cd %end; from &fltype._method3bo where dgns_null=0 limit 100);
	...
	** Join the inpatient bene-level table with the outpatient bene/date-level table (after aggregating to bene-level)
	...
	title2 "Creation of POP_SUD_NODX - bene-level table with identification of SUD without diagnosis";

	%crosstab(BENE_SUD_NODX,POP_SUD_NODX)
	%crosstab(BENE_SUD_NODX,POP_SUD_NODX INP_SUD OUTP_SUD2)

	title2 "Creation of OUD_NODX - bene-level table with identification of OUD without diagnosis";

	%crosstab(BENE_SUD_NODX,OUD_NODX)
	%crosstab(BENE_SUD_NODX,OUD_NODX INP_OUD OUTP_OUD2)
	%crosstab(BENE_SUD_NODX,OUD_NODX POP_SUD_NODX) 
